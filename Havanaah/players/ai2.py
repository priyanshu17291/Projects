...
class MCTS: 
    ...
    def __init__(self, state: np.array, player_number: int, p1: list[list[list[Tuple[int, int]]]], try_s: int, confidence: float, history) -> None:
        self.state = state
        self.dim = len(state)
        self.player_number = player_number
    ...
    def choose(self, state: np.array, it: int) -> Tuple[int, int]:
        ...
        for child in self.root.children.values():
            ...
            history[child_state_tuple]=(child.n,child.t)
            for grandchild in child.children.values():
                grandchild_state_tuple = tuple(map(tuple, grandchild.state))
                history[grandchild_state_tuple] = (grandchild.n, grandchild.t)
    ...
    def pick(self,node:Node)->float:
        ...
        return ucb*(1+(2.0/(1.0+self.confidence))*(0.6*connection+1.2*group)*(1+0.5*local))
...
class AIPlayer:
    ...
    def __init__(self, player_number: int, timer):
        ...
        self.history=defaultdict(np.array)
    ...
    def get_move(self, state: np.array) -> Tuple[int, int]:
        ...
        tree=MCTS(state,self.player_number,self.starters[self.dim],self.try_s,self.confidence,self.history)
        iters=int(fetch_remaining_time(self.timer,self.player_number)*9)
        move,history= tree.choose(state,iters)
